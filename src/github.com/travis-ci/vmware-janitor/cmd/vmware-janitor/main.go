...
func main() {
	...
	vmPath := os.Getenv("VSPHERE_VM_PATH")
	if vmPath == "" {
		log.Fatal("missing VSPHERE_VM_PATH")
	}
	...
	cutoffDurationString := os.Getenv("CUTOFF")
	if cutoffDurationString != "" {
		dur, err := time.ParseDuration(cutoffDurationString)
		if err != nil {
	...
	if concurrencyString != "" {
		val, err := strconv.ParseUint(concurrencyString, 10, 64)
		if err != nil {
			log.Fatalf("ERROR: %v", err)
		}
		concurrency = val
	...
	log.Printf("Using concurrency %v", concurrency)
	sem := make(chan struct{}, int(concurrency))
